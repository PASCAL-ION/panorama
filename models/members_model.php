...
function getUsers($lastname, $firstname){
    $con = BDConnection();
    if ($con) {
        $request = 'SELECT lastname, firstname, subscription.name AS subscriptionPlan, user.id AS userid FROM user LEFT JOIN membership ON membership.id_user=user.id LEFT JOIN subscription ON subscription.id=membership.id_subscription';
